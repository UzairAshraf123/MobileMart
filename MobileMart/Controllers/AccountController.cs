...
namespace MobileMart.Controllers
{
    ...
    public class AccountController : HomeBaseController
    {
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> OwnerRegister1(CreateOwnerViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email };
                ...
                if (result.Succeeded)
                {
                    ...
                    model.UserID = user.Id;
                    adminBl.CreateOwner(model);
                    return RedirectToAction("CreateShop", "Home", new { userID = user.Id });
            ...
            // If we got this far, something failed, redisplay form
            return RedirectToAction("Index", "Home",new { message= "Model state is not valid.."});
        }
        [HttpPost]
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
